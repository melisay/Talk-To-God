...
class PersonalityManager:
    def __init__(self, tts_manager=None):
        ...
        self.current_personality = "nikki"  # always store the name
        # make sure we have something to roll back *before* first switch
        self._old_personality = self.current_personality
    ...
    def set_personality(self, new_personality: str) -> None:
        ...
        if new_personality not in self.personalities:
            raise ValueError(f"Unknown personality: {new_personality}")
        ...
        log_structured_data(
            ...
            "personality_switch",
            {
                ...
                "from_personality": self._old_personality,
                "to_personality":   new_personality
            }
        )
        # now stash the old and switch
        self._old_personality     = self.current_personality
        ...
        if new_personality == "nikki":
            ...
            try:
                ...
                pass
        if self.tts_manager:
            self.tts_manager.set_voice(self.personalities[new_personality].voice_id)
    
    def get_current_personality(self) -> Personality:
        """Get the current personality."""
        return self.personalities[self.current_personality]  # Convert name to object
    ...
    def check_easter_egg(self, raw_text: str) -> Optional[str]:
        """Check if the input text matches any easter egg phrases."""
        # Clean and lowercase the input text
        text = re.sub(r'[^\w\s]', '', raw_text).strip().lower()
        personality = self.get_current_personality()
        
        # Check for any trigger word in the cleaned text
        for trigger, response in personality.easter_eggs.items():
            if trigger in text:
                return response
        return None
