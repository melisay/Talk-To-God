"""Entertainment features for the AI Assistant project."""
...
class EntertainmentManager:
    ...
    def get_fact(self) -> str:
        """Get a random fact."""
        fact = choice(self.facts)
        logger.debug("Selected fact")  # Avoid logging the entire fact for security
        return fact
    
    def validate_category(self, category: str) -> bool:
        """Validate that the requested category is allowed."""
        valid_categories = ["joke", "riddle", "story", "fact"]
        return category in valid_categories
    
    def get_entertainment(self, category: str, user_id: Optional[str] = None) -> Optional[Dict[str, str]]:
        """Get random entertainment content based on category."""
        # Validate category to prevent authorization bypass
        if not self.validate_category(category):
            logger.warning(f"Invalid entertainment category requested: {category}")
            return None
            
        # Additional authorization check if needed
        if user_id and not self._check_user_authorization(user_id):
            logger.warning(f"Unauthorized entertainment request from user: {user_id}")
            return None
    ...
    def _check_user_authorization(self, user_id: str) -> bool:
        """Check if user is authorized for entertainment features."""
        # For now, allow all users - can be extended with actual auth logic
        return True
